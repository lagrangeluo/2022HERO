...
float FricLeftSpeed[5] 	= {5.0f,			0.6f,			4.0f,				15000.0f,			500.0f};  //左摩擦轮速度环
float FricRightSpeed[5] = {5.0f,			0.6f,			4.0f,				15000.0f,			500.0f};  //右摩擦轮速度环
int16_t ttt;
...
void shoot_task(void)
{
	...
	if(rc_shoot.trigger.target_angle == 1)
	{
	...
	else if(rc_shoot.trigger.target_angle == 0)	
		canTX_trigger(rc_shoot.trigger.target_angle);
//		if(canTX_trigger(rc_shoot.trigger.target_angle))
//			rc_shoot.trigger.target_angle = 0;
